...
func NewRemoveHorizontalWallMapCommandFactory() MapCommandFactory {
	return NewMapCommandFactory(NewRemoveHorizontalWallMapCommand, func(m *maps.TileMap, row, col int) []TileIndex {
		if m.GetAllBits(row, col, maps.FLOOR_BIT, maps.INTERIOR_WALL_S_BIT) && m.GetAllBits(row+1, col, maps.FLOOR_BIT, maps.INTERIOR_WALL_N_BIT) {
